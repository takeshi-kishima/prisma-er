...
export class ExtensionConfig {
  ...
  constructor(configSession: string) {
    this.config = workspace.getConfiguration(configSession);
  }
  ...
  getPreferredTheme(): Theme {
    ...
    if (Theme.light === preferredTheme) return preferredTheme;
    return Theme.dark;
  }

  getScrollDirection(): ScrollDirection {
    const scrollDirection = this.config.get(ConfigKeys.scrollDirection);
    if (ScrollDirection.UpIn === scrollDirection) return scrollDirection;
