...
import { type TableDetailLevelValue, TableDetailLevel } from "@/visualizer/types/tableDetailLevel";
...
const TableLevelDetailProvider = ({ children }: TableLevelDetailProviderProps) => {
  ...
  const next = useCallback((): void => {
    if (state === TableDetailLevel.FullDetails) setState(TableDetailLevel.HeaderOnly);
    else if (state === TableDetailLevel.HeaderOnly) setState(TableDetailLevel.KeyOnly);
    else setState(TableDetailLevel.FullDetails);
  }, [state, setState]);
  return (
