...
export const TablesPositionsContext = createContext<TablesPositionsContextValue | null>(null);
...
const TablesPositionsProvider = ({ tables, refs, children }: TablesPositionsProviderProps) => {
  const resetPositions = () => { tableCoordsStore.resetPositions(tables, refs); };
  const contextValue = useMemo(() => ({ resetPositions }), [resetPositions]);
  return <TablesPositionsContext.Provider value={contextValue}>{children}</TablesPositionsContext.Provider>;
};
