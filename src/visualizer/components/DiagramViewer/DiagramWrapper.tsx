...
import { useWindowSize } from "@/visualizer/hooks/window";
import { useCursorChanger } from "@/visualizer/hooks/cursor";
import { DIAGRAM_PADDING } from "@/visualizer/constants/sizing";
import { useThemeColors } from "@/visualizer/hooks/theme";
import { useStageStartingState } from "@/visualizer/hooks/stage";
import { stageStateStore } from "@/visualizer/stores/stagesState";
import { useScrollDirectionContext } from "@/visualizer/hooks/scrollDirection";
import { ScrollDirection } from "@/visualizer/types/scrollDirection";
...
const DiagramWrapper = ({ children }: DiagramWrapperProps) => {
  ...
  const { onChange: onGrabbing, onRestore: onGrabRelease } = useCursorChanger("grabbing");
  const themeColors = useThemeColors();
  const stageRef = useRef<null | CoreStage>(null);
  const { hoveredTableName, setHoveredTableName, highlightedColumns, setHighlightedColumns } = useTablesInfo();

  const { scale: defaultStageScale, position: defaultStagePosition } = useStageStartingState();
  useEffect(() => {
    if (stageRef.current != null) {
      stageRef.current.scale({ x: defaultStageScale, y: defaultStageScale });
  ...
  const handleZooming = (e: KonvaEventObject<WheelEvent>) => {
    ...
    let direction = 0;
    if (scrollDirection === ScrollDirection.UpOut) direction = e.evt.deltaY > 0 ? 1 : -1;
    else if (scrollDirection === ScrollDirection.UpIn) direction = e.evt.deltaY > 0 ? -1 : 1;
  ...
  const nodeBelongsToTable = (node: any): boolean => {
    ...
    while (currentNode != null) {
      if (typeof currentNode.name === "function" && typeof currentNode.name() === "string") {
        const names = (currentNode.name() as string).split(/\s+/);
        if (names.some((n: string) => n.startsWith("table-"))) return true;
      }
  ...
  const handleStagePointerDown = (e: KonvaEventObject<MouseEvent | TouchEvent>) => {
    if (hoveredTableName == null && (highlightedColumns == null || highlightedColumns.length === 0)) return;
  ...
  const fitToView = () => {
    if (stageRef.current != null) {
      ...
      contentBounds.x -= DIAGRAM_PADDING;
      contentBounds.y -= DIAGRAM_PADDING;
      contentBounds.width += 2 * DIAGRAM_PADDING;
      contentBounds.height += 2 * DIAGRAM_PADDING;
      const scaleX = containerWidth / contentBounds.width;
      const scaleY = containerHeight / contentBounds.height;
      const scale = Math.min(scaleX, scaleY);
      stage.scale({ x: scale, y: scale });
      stage.position({
        x: (containerWidth - contentBounds.width * scale) / 2 - contentBounds.x * scale,
        y: (containerHeight - contentBounds.height * scale) / 2 - contentBounds.y * scale,
      });
      stage.batchDraw();
  ...
  useEffect(() => {
    const handler = ({ tableName }: { tableName: string }) => {
      ...
      try {
        (stage as any).to({
          x: newPos.x, y: newPos.y, duration: 0.45,
          onFinish: () => { stage.batchDraw(); stageStateStore.set({ scale: stage.scaleX(), position: newPos }); },
        });
      } catch {
        stage.position(newPos);
        stage.batchDraw();
    ...
    return () => { eventEmitter.off("table:center", handler); };
  }, []);
  ...
  const onDownload = () => {
    ...
    const data = stage.toDataURL({
      x: contentBounds.x - offsetX, y: contentBounds.y - offsetY,
      width: maxDimension, height: maxDimension, pixelRatio: 2,
    });
  ...
  return (
    <>
      <Stage draggable ref={stageRef} onDragStart={onGrabbing} onDragEnd={onGrabRelease} onWheel={handleZooming} onMouseDown={handleStagePointerDown} onTouchStart={handleStagePointerDown} width={windowWidth} height={windowHeight} style={{ width: "fit-content", backgroundColor: themeColors.bg }}>
        <Layer>
