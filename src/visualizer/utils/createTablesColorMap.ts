...
export const createTablesColorMap = (tables: JSONTableTable[]): Map<string, TableColors> => {
  const tableColors = new Map<string, TableColors>();
  tables.forEach((table) => {
    const tableColor = !!table.headerColor
      ? { regular: table.headerColor, lighter: getContrastColor(table.headerColor) }
